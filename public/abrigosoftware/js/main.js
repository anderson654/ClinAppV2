$(document).ready(function () {
    ...
    $(document).on( 'init.dt', function ( e, settings ) {
        if (typeof window.route_mass_crud_entries_pay != 'undefined') {
            $('.datatable, .ajaxTable').siblings('.actions').html('<a href="' + window.route_mass_crud_entries_pay + '" class="btn btn-xs btn-warning js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">'+window.payButtonTrans+'</a>');
        }
    });
    ...
    $(document).on('click', '.js-delete-selected', function () {
        if (confirm('Are you sure')) {
            ...
            $(this).closest('.actions').siblings('.datatable, .ajaxTable').find('tbody tr.selected').each(function () {
                console.log("selected1", $(this).data('entry-id'));
                console.log("selected1", $(this).data('entry-id'));
...
function processAjaxTables() {
    $('.ajaxTable').each(function () {
        ...
        $(this).DataTable(window.dtDefaultOptions);
        if (typeof window.route_mass_crud_entries_pay != 'undefined') {
            $(this).siblings('.actions').html('<a href="' + window.route_mass_crud_entries_pay + '" class="btn btn-xs btn-warning js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">'+window.payButtonTrans+'</a>');
        }
    });
