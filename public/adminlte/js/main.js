$(document).ready(function () {
    ...
    $(document).on( 'init.dt', function ( e, settings ) {
        if (typeof window.route_mass_crud_entries_destroy != 'undefined') {
            $('.datatable, .ajaxTable').siblings('.actions').html('<a href="' + window.route_mass_crud_entries_destroy + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Excluir Selecionados</a>');
        }
    });

    $(document).on( 'init.dt', function ( e, settings ) {
        if (typeof window.route_mass_crud_entries_pay != 'undefined') {
            $('.datatable, .ajaxTable').siblings('.actions').html('<a href="' + window.route_mass_crud_entries_pay + '" class="btn btn-xs btn-warning js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Pagar Selecionados</a>');
        }
    });
    ...
    $(document).on('click', '.js-delete-selected', function () {
        if (confirm('Tem certeza que deseja deletar os itens selecionados?')) {
            ...
            $(this).closest('.actions').siblings('.datatable, .ajaxTable').find('tbody tr.selected').each(function () {
                console.log("teste", $(this).data('entry-id'));
    ...
    $(document).on('click', '.js-pay-selected', function () {
        if (confirm('Tem certeza que deseja executar os pagamentos selecionados?')) {
            var ids = [];
...
function processAjaxTables() {
    $('.ajaxTable').each(function () {
        ...
        $(this).DataTable(window.dtDefaultOptions);
        if (typeof window.route_mass_crud_entries_pay != 'undefined') {
            $(this).siblings('.actions').html('<a href="' + window.route_mass_crud_entries_pay + '" class="btn btn-xs btn-warning js-pay-selected" style="margin-top:0.755em;margin-left: 20px;">'+window.payButtonTrans+'</a>');
        }
    });
